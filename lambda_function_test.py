...
def updateStationsFile(client, list):
    ...
    for l in list:
        ...
        lastline = ''
        try:
            obj = client.get_object(Bucket='p3.dlarah.org', Key=datafile)
        ...
        if len(lastline) == 0 or lastline[5] != l.time:
            ...
            client.put_object(Body=data, Bucket='p3.dlarah.org', Key=datafile, ACL='public-read', ContentType='text/plain;charset=utf-8')
        else:
            print("duplicate line not added", lastline, l)
        
    client.put_object(Body=stationsData, Bucket='p3.dlarah.org', Key='wind_data/stations.txt', ACL='public-read', ContentType='text/plain;charset=utf-8')
...
def lambda_handler(event, context):
    ...
    obj = client.get_object(Bucket='p3.dlarah.org', Key='fmi_api_key.txt')

    (htmlCode, list) = winds_lib.gatherAllStationData(obj['Body'].read())
    for l in htmlCode:
        windData += l + "\n"
    client.put_object(Body=windData, Bucket='p3.dlarah.org', Key='winds.html', ACL='public-read', ContentType='text/html;charset=utf-8')
    ...
    for l in htmlCode:
        windData += l + "\n"
    client.put_object(Body=windData, Bucket='p3.dlarah.org', Key='winds_ee.html', ACL='public-read', ContentType='text/html;charset=utf-8')
